...
def main(args):
    """
    Main function for training an EfficientNet model and evaluating it on different datasets.
    ...
    train_data_dir = f"E:/data/{args.TRAINING_DATA}" + "/train_data" 
    val_data_dir = f"E:/data/{args.TRAINING_DATA}" + "/validation_data"
    ...
    num_classes = 2
    effnet = EfficientNet.from_pretrained('efficientnet-b2', num_classes=num_classes)
    ...
    optimizer = torch.optim.Adam(effnet.parameters(), lr=0.0001)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        # Save model
        torch.save(effnet.state_dict(), save_path_model + f"/effnet_{args.TRAINING_DATA}.pt")

    # Test loop rsna
    test_data_dir = "E:/data/rsna/test_data"
    ...
    with torch.no_grad():
        for test_inputs, test_labels in test_loader:
            ...
            test_outputs = effnet(test_inputs)
    ...
    # Test loop vindir
    test_data_dir = "E:/data/vindir/test_data"
    test_loader = dataloader.load_dataset(test_data_dir)
    effnet.eval()
    test_correct_predictions = 0
    test_total_samples = 0
    ...
    # Test loop ddsm
    test_data_dir = "E:/data/ddsm/test_data"
    test_loader = dataloader.load_dataset(test_data_dir)
    effnet.eval()
    test_correct_predictions = 0
    test_total_samples = 0
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(description='Effnet training')
